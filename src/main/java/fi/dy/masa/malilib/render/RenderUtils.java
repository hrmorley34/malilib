        ...
        for (String line : lines)
        {
            ...
            if (line.matches(".*§x#[0-9A-Fa-f]{6}.*"))
            {
                ...
                while (matcher.find(searchIndex))
                {
                    ...
                    if (start != end)
                    {
                        cx = drawContext.drawText(fontRenderer, lastFormat+line.substring(start, end), cx, y, lastColour, useShadow);
                    }
                ...
                {
                    cx = drawContext.drawText(fontRenderer, lastFormat + line.substring(searchIndex), cx, y, lastColour, useShadow);
                }
            }
            else
            {
